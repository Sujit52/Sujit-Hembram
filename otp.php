...
<?php
  ...
  error_reporting(0);
   ...
   $xref=$_REQUEST['xref'];
   $gatkn=$_REQUEST['gatkn'];
    $otp=$_REQUEST['otp'];
    $usrtkn=$_REQUEST['usrtkn'];
...
$url="https://cricket6.in/user/auth/validate_otp";
    
    $data='{"otp":"'.$otp.'","phone_no":"'.$mobile.'","facebook_access_token":"","facebook_id":"","google_access_token":"","google_id":"","device_type":"3","device_id":""}';
...
if($code==="Your phone number verified successfully.")
{
	
	
echo"<div class='msg success'>Enter Your Details</div>";
echo"<form action='verify.php' method='POST'>
	<input type='hidden' value='$mobile' name='mobile'> 
		<input type='hidden' value='$sessionkey' name='sessionkey'> 
			<input type='text' name='bankname' class='input' placeholder='Enter Bank Name' required>
    <input type='int' name='bankno' class='input' placeholder='Enter Bank Number' required>
    	    <input type='text' name='ifsc' class='input' placeholder='Enter IFSC Code' required>
    <input type='submit' name='submit' class='button shine' value='Claim Amount '>
        </form>";
